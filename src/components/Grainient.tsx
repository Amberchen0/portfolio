...
/**
 * Grainient — React Bits canonical component, ported to TSX.
 *
 * WebGL2 fullscreen quad rendering a slowly-warping three-colour
 * gradient + procedural film grain overlay. Heavily tunable: timeSpeed,
 * warp shape, blend, rotation, noise, grain, contrast/gamma/saturation,
 * zoom + centre offset.
 *
 * Architecture note (from React Bits): renderer/program live in a
 * module-level WeakMap keyed by the container DIV, so the WebGL
 * context is built ONCE per mount and prop changes only update
 * uniforms — re-renders never tear down the GL context. IntersectionObserver +
 * visibilitychange pause the rAF loop when the canvas is off-screen
 * or the tab is hidden, so it's cheap when not visible.
 *
 * Sized 100%×100% of its parent — wrap it in a positioned container.
 */
